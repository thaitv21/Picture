...
import com.panasonic.cdd.imagesegmentationlib.R;
...
public class DeepLabLite implements DeeplabInterface {
    ...
    public boolean initialize(Context context) {
        ...
        if (mModelBuffer == null) {
            Log.d("DeepLabLite", "initialize: buffer null");
            return false;
        }
    ...
    private static MappedByteBuffer loadModelFile(Context context, String modelFile) {
        if (context == null || TextUtils.isEmpty(modelFile)) {
            Log.d("DeepLabLite", "context = null || modelFile = empty");
            return null;
        }
        ...
        try {
            Log.d("DeepLabLite", "loadModelFile: " + Arrays.toString(context.getAssets().list("")));
            AssetFileDescriptor df = context.getResources().openRawResourceFd(R.raw.deeplabv3_257_mv_gpu);
            ...
            Log.d("DeepLabLite", "buffer: " + buffer);
        } catch (IOException e) {
            Log.d("DeepLabLite", "error: " + e.getMessage());
